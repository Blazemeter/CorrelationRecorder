...
import static com.blazemeter.jmeter.correlation.core.templates.TemplateUtils.TEST_TEMPLATE_DESCRIPTION;
import static com.blazemeter.jmeter.correlation.core.templates.TemplateUtils.TEST_TEMPLATE_NAME;
import static com.blazemeter.jmeter.correlation.core.templates.TemplateUtils.TEST_TEMPLATE_VERSION;
...
public class LocalConfigurationTest extends WiredBaseTest {
  ...
  private static final String TEST_REPO_NAME = TEST_TEMPLATE_NAME;
  ...
  @Mock
  private CorrelationTemplatesRepository expectedLocalRepository;
  ...
  public void setUp() throws IOException {
    ...
    dependenciesFolder = JMeterUtils.getJMeterHome() + "/lib/ext/";
    prepareExpectedCentralRepository();
    mockExpectedLocalRepository();
  }
  ...
  private void prepareExpectedCentralRepository() {
    when(expectedCentralRepository.getValues()).thenReturn(
        CORRELATION_TEMPLATES_REPOSITORY_NAME + "{name='" + CENTRAL_REPOSITORY_NAME
            + "', displayName='" + CENTRAL_REPOSITORY_DISPLAY_NAME + "', templatesVersions={"
            + WORDPRESS_TEMPLATE_REFERENCE_NAME + "=CorrelationTemplateVersions {versions=["
            + TEMPLATE_VERSION_THREE + "]}}}");

  }

  private void mockExpectedLocalRepository() {
    when(expectedLocalRepository.getValues()).thenReturn(
        CORRELATION_TEMPLATES_REPOSITORY_NAME + "{name='" + "local"
            + "', displayName='" + "Local" + "', templatesVersions={}}");
  }
  ...
  public void shouldReturnRepositoryNameWhenAdded() {
    assertEquals(Arrays.asList(CENTRAL_REPOSITORY_NAME, localRepository.getName(),
        firstRepository.getName()), configuration.getRepositoriesNames());
  }
  ...
  public void shouldReturnOnlyRepositoriesWithInstalledTemplateWhenGetRepositoriesWithInstalledTemplates()
    ...
    configuration.manageTemplate(LocalConfiguration.INSTALL, firstRepository.getName(),
        firstTemplateID, firstTemplateVersion);

    List<CorrelationTemplatesRepositoryConfiguration> expected = Arrays.asList(
        localRepositoryConfiguration, repositoryWithInstalledTemplates);

    List<CorrelationTemplatesRepositoryConfiguration> actual =
        configuration.getRepositoriesWithInstalledTemplates();

    //We are taking the second since the first it's the local repository with the Custom template
    // added
    assertTrue(expected.size() == actual.size() && actual.get(1).equals(expected.get(1)));
  }

  private void installTestTemplate() throws IOException, ConfigurationException {
    TemplateUtils.installTestTemplate(folder.getRoot().toPath().toAbsolutePath().toString(),
        configuration);
  }
  ...
  private void prepareExpectedRepositoryWithInstalledTemplates() {
    ...
    when(repositoryWithInstalledTemplates.getInstalledTemplates()).thenReturn(
  ...
  public void shouldReturnTrueWhenIsInstalledWithInstalledTemplate() throws ConfigurationException {
    configuration.manageTemplate(LocalConfiguration.INSTALL, firstRepository.getName(),
        firstTemplateID, firstTemplateVersion);
    assertTrue(configuration.isInstalled(firstRepository.getName(), firstTemplateID,
        firstTemplateVersion));
  }

  @Test
  public void shouldReturnFalseWhenIsInstalledWithNotInstalledTemplate() {
    assertFalse(configuration.isInstalled(firstRepository.getName(), firstTemplateID,
        firstTemplateVersion));
  }

  @Test(expected = ConfigurationException.class)
  public void shouldThrowAnExceptionWhenManageTemplateWithNotAddedRepo()
      throws ConfigurationException {
    configuration.manageTemplate(LocalConfiguration.INSTALL, secondRepository.getName(),
        firstTemplateID, firstTemplateVersion);
  }

  @Test
  public void shouldUninstallTemplateWhenManageTemplateWithEmptyAction()
      throws ConfigurationException {
    configuration.manageTemplate(LocalConfiguration.INSTALL, firstRepository.getName(),
        firstTemplateID, firstTemplateVersion);

    configuration.manageTemplate("", firstRepository.getName(), firstTemplateID,
        firstTemplateVersion);
    assertFalse(configuration.isInstalled(firstRepository.getName(), firstTemplateID,
        firstTemplateVersion));
  }
  ...
  @Test
  public void shouldInstallTemplateWhenInstallTemplate()
      throws ConfigurationException, IOException {
    prepareRepoFiles();
  ...
  public void shouldUninstallTemplateWhenUninstallTemplate() throws ConfigurationException {
    configuration.manageTemplate(LocalConfiguration.INSTALL, firstRepository.getName(),
        firstTemplateID, firstTemplateVersion);
    configuration.uninstallTemplate(firstRepository.getName(), firstTemplateID,
        firstTemplateVersion);
    assertFalse(configuration.isInstalled(firstRepository.getName(), firstTemplateID,
        firstTemplateVersion));
  }
  ...
  public void shouldReturnListWithRepositoriesWhenGetRepositoriesNames() {
    configuration.addRepository(secondRepository.getName(), secondRepositoryURL);
    assertEquals(Arrays.asList(CENTRAL_REPOSITORY_NAME, localRepository.getName(), FIRST_REPO_NAME,
        SECOND_REPO_NAME), configuration.getRepositoriesNames());
  }
  ...
  @Test
  public void shouldReturnTrueWhenRefreshRepositoryWithUpToDataRemoteRepoAndLocal()
    ...
    assertEquals(Arrays.asList("Syncing 'test' repository.", "Syncing 'local' repository.",
        "Updating 'Local' repository.",
        "Updating 'http://localhost:" + getWiredPort() + TEST_REPOSITORY_URL + "' repository.",
        "Update finished"), statusList);
  }
  ...
  public void shouldReturnInstalledTemplatesWhenGetInstalledTemplates()
    ...
    Assert.assertEquals(installedTemplates.get(0).getId(), TEST_TEMPLATE_NAME);
    Assert.assertEquals(installedTemplates.get(0).getDescription(), TEST_TEMPLATE_DESCRIPTION);
    Assert.assertEquals(installedTemplates.get(0).getVersion(), TEST_TEMPLATE_VERSION);
  }


  @Test
  public void shouldReturnTrueWhenIsValidDependencyURLWithValidURL() throws IOException {
    startWiredMock(WIRED_HOST);
    mockingResponse(TEST_REPOSITORY_URL, 200,
        TestUtils.getFileContent(TEST_REPOSITORY_URL, getClass()), "head");
    boolean isValid = configuration.isValidDependencyURL(getBaseURL() + TEST_REPOSITORY_URL,
        TEST_REPO_NAME, "1.0");
  ...
  public void shouldReturnFileNameForTemplateWhenGetTemplateFilename()
      throws ConfigurationException, IOException {
    installTestTemplate();
    Template customTemplate = configuration.getInstalledTemplates().get(0);
    String fileName = LocalConfiguration.getTemplateFilename(customTemplate);
    Assert.assertEquals(TEST_TEMPLATE_NAME + "-" + TEST_TEMPLATE_VERSION + "-template.json",
        fileName);
  }
  ...
  public void shouldReturnRepoFolderWhenGetRepositoryFolder() throws IOException {
    ...
    Assert.assertEquals(
        Paths.get(folder.getRoot().getPath(), "correlation-templates", TEST_REPO_NAME)
            .toAbsolutePath().toString(), repoFolder.getAbsolutePath());
  }

  @Test
  public void shouldReadTemplatePropertiesWhenReadTemplatePropertiesFromPath() throws IOException {
    prepareRepoFiles();
    Path templatePath = Paths.get(folder.getRoot().getPath(), "correlation-templates",
        TEST_REPO_NAME, firstTemplateID + "-" + firstTemplateVersion + "-properties.json");
    TemplateProperties templateprop = configuration.readTemplatePropertiesFromPath(
  ...
  private void prepareDependenciesFolder() {
    new File(dependenciesFolder).mkdirs();
  }
  ...
  private List<String> getInstalledDependencies() {
    return Arrays.stream(Objects.requireNonNull(new File(dependenciesFolder).list()))
        .filter(f -> f.toLowerCase().endsWith(JAR_SUFFIX)).collect(Collectors.toList());
  }
  ...
  public void shouldFindJarsWhenGetJarFileByConditionWithNameContains() throws IOException {
    prepareDependenciesFolder();
    createSingleDependency(1);
    assertArrayEquals(buildExpectedDependencies(),
        configuration.getJarFileByCondition(dependenciesFolder,
            (fileName) -> fileName.contains(VERSION_ONE)));
  }
  ...
  private List<CorrelationTemplatesRepository> prepareExpectedLocalRepositories() {
    ...
    expectedLocalRepositories.add(expectedLocalRepository);
