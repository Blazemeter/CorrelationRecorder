...
public class RulePanel extends JPanel implements Serializable {
  ...
  private PlaceHolderTextField referenceVariableField;
  ...
  private void setUpComboBox(ItemEvent e, boolean isReplacement, String ruleId,
    ...
    correlationParametersDefinition.forEach(p -> {
      ...
      Component field = isComboBox ? new JComboBox() : new PlaceHolderTextField();
      ...
      if (isComboBox) {
        ...
        ((PlaceHolderTextField) field).setToolTipText(p.getDescription());
        ((PlaceHolderTextField) field)
            .setPlaceHolder(removeCorrelationRulePropertyPrefix(p.getName()));
        ((PlaceHolderTextField) field)
            .setText(!p.getDefaultValue().isEmpty() ? p.getDefaultValue() : "");
      }
